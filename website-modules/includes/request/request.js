...
function request_get_styles_and_scripts(response_obj) {
  var response_html = new DOMParser().parseFromString(
    response_obj.responseText,
    "text/html"
  );
  ...
  for (let i = 0; i < response_html_styles.length; i++) {
    document
      ...
      .appendChild(response_html_styles[i]);
  }
  ...
  return Promise.resolve(
    "Styles and scripts added from " + response_obj.responseURL
  );
}
function request_response_HTML(response_obj) {
  let response_html = new DOMParser()
    .parseFromString(response_obj.responseText, "text/html")
