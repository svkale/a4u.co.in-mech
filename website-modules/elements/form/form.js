function form_JSON_send(form) {
  let form_submit_url = form.getAttribute("data-form-submit-target"),
    form_params_obj = form_JSON_string_data(form);
  ...
  request_post_promise(
    form_submit_url,
    JSON.stringify(form_params_obj),
    2
  ).then(
    (res) => {
      //console.log(res);
      if (form.getAttribute("data-custom-JSON-to-send-file")) {
        form_JSON_file_data(form).then(
          (file) => {
            var id = JSON.parse(res.response).id,
              info = JSON.parse(res.response).information_status;
            console.log(
              form_submit_url,
              JSON.stringify({
                ...file,
                ...{ id: id, information_status: info },
              })
            );
            request_post_promise(
              form_submit_url,
...
function form_JSON_string_data(form) {
  ...
  for (let form_element of form) {
    if (form_element.tagName == "SELECT") {
      ...
      if (form_element.type == "text") {
        ...
        form_str_JSON["input"]["date"][form_element.name] = form_element.value;
        ...
        if (form_element.checked == true) {
          ...
          form_str_JSON["input"]["checkbox"]["unchecked"].push(
            form_element.name
          );
        }
      } else if (form_element.type == "password") {
        form_str_JSON["input"]["password"][form_element.name] =
          form_element.value;
      }
    }
  }
  ...
  if (form.getAttribute("data-custom-JSON-to-send-string")) {
    return {
      ...form_str_JSON,
      ...JSON.parse(form.getAttribute("data-custom-JSON-to-send-string")),
    };
  } else {
...
function form_JSON_file_data(form) {
  return new Promise((resolve, reject) => {
    var form_file_JSON = {},
      form_files = form.querySelectorAll('input[type="file"]');
    for (let j = 0; j < form_files.length; j++) {
      ...
      for (let i = 0; i < form_file_all.length; i++) {
        ...
        filereader.addEventListener("load", function () {
          form_file_JSON[form_files[j].name][i]["file_bytes"] = [
            ...new Int8Array(this.result),
          ];
          form_file_JSON[form_files[j].name][i]["file_mimetype"] =
            form_file_all[i].type;
          form_file_JSON[form_files[j].name][i]["file_size"] =
            form_file_all[i].size;
          count++;
          if (count == climit) {
            if (form.getAttribute("data-custom-JSON-to-send-file")) {
              return resolve({
                ...form_file_JSON,
                ...JSON.parse(
                  form.getAttribute("data-custom-JSON-to-send-file")
                ),
              });
            } else {
...
function form_add_input(event, list) {
  ...
  let number = parseInt(list.getAttribute("data-form-input-number")) + 1,
    html = list.getAttribute("data-form-input-template");
  list.setAttribute("data-form-input-number", number);
  list
    .getElementsByTagName("button")[0]
    .insertAdjacentHTML("beforebegin", html.replace(/%%num%%/g, number));
  return;
}
function form_remove_input(event, list) {
  event.preventDefault();
  let number = parseInt(list.getAttribute("data-form-input-number")),
    min_number = parseInt(list.getAttribute("data-form-input-min-number"));
  ...
  list.querySelectorAll(
    "input[name='" +
      list.getAttribute("data-form-input-name") +
      "_" +
      number +
      "']"
...
function form_store_creds(obj) {
  if (sessionStorage) {
    ...
    document.getElementById("form_creds").innerHTML = JSON.stringify({
...
function form_get_creds() {
  if (sessionStorage) {
    let form_creds_obj = {
      ...
      rolekey: sessionStorage.getItem("rolekey"),
...
function form_wait(form) {
  ...
  response_div.insertAdjacentHTML(
    "beforeend",
    "<h4 align='center'>Please wait!<br>Your form is being processed</h4>"
  );
  return;
}
function form_finalize(response) {
  ...
  if (
    response_div.getAttribute("data-show-id") == undefined ||
    response_div.getAttribute("data-show-id") != "no"
  ) {
    response_div.insertAdjacentHTML(
      "beforeend",
      "The 'id' for your request is: <strong>" +
        response["id"] +
        "</strong>.<br>"
    );
  }
  if (response["information_status"]) {
    response_div.insertAdjacentHTML(
      "beforeend",
      response["information_status"] + "<br>"
    );
  }
  if (response["res"]) {
    if (
