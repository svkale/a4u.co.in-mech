...
function load_notice_as_page(id) {
      ...
      if (JSON.parse(res.response)["notice_error"]) {
        ...
        document
          ...
          .insertAdjacentElement(
            "beforeend",
            notice_parse(JSON.parse(res.response)["notice"])
          );
      }
    },
    (error) => {
  ...
  request_promise(
    "https://script.google.com/macros/s/AKfycbw9eXxMIO6sw6CtNa7APdyeuFdiijxLlGpIe8UYY47LPGhitGU4qqbzOHnhIzy_s1oO3w/exec?fn=hitCounter&site=a4u.co.in"
  ).then((res) => {});
}
...
function notice_board_replace(notice_board_heading) {
  ...
  notices_board.innerHTML = notice_board_post(
    notice_board_heading.substring(0, notice_board_heading.length - 8)
  );
}
function notice_board_paste(notice_board_heading) {
  ...
  if (notices_obj[p + "_page_number"]) {
    notices_board.insertAdjacentHTML(
      "beforeend",
      '<div align="center"><button><span>Previous Page</span></button><button><span>Next Page</span></button></div>'
    );
  }
  document
    .getElementById(target_ele_id)
    .insertAdjacentElement("afterbegin", notices_board);
}
function notice_board_post(p) {
  const notices = notices_obj[p + "_notices"],
    notices_count = notices_obj[p + "_notices_count"];
  let notices_str =
    "<button onclick=\"let nav_opts = document.querySelectorAll('#contents_nav li');for(let i of nav_opts) { if(i.innerText == '" +
    p +
    '\') { i.click(); } }" style="background: revert; border: 0; width: 100%; text-decoration: underline; padding: 0;cursor: pointer;"><h4 style="color: var(--main-color); background-color: var(--main-color-2);">Notices (' +
    p.replaceAll("_", " ") +
    ")</h4></button>";
  for (let i = notices.length - 1; i >= 0; i--) {
    notices_str +=
      "<article class='cont1 notice'><big><b onclick='notice_show_with_no(event.target.getAttribute(\"data-notice-group\"),event.target.getAttribute(\"data-notice-number\"));' data-notice-group='" +
      p +
      "_notices" +
      "' data-notice-number='" +
      i +
      "'>" +
      notices[i][4] +
      "</b></big><hr><div class='notices_author'>by " +
      notices[i][1] +
      "</div><div class='notices_date'>on " +
      notices[i][2].slice(notices[i][2].length - 2) +
      "/" +
      notices[i][2].slice(notices[i][2].length - 4, notices[i][2].length - 2) +
      "/" +
      notices[i][2].slice(0, notices[i][2].length - 4) +
      "</div><br><div class='notices_for'>The notice is for " +
      notice_get_students_group(notices[i][5]) +
      ".</div></article>";
  }
  return (
    notices_str +
    "<hr style='float: left;'><button onclick=\"let nav_opts = document.querySelectorAll('#contents_nav li');for(let i of nav_opts) { if(i.innerText == '" +
    p +
    "') { i.click(); } }\" style=\"background: revert; border: 0; width: 100%; text-decoration: underline; padding: 0;cursor: pointer;\"><div class='notices_count'>Total " +
    notices_count +
    " notices found.</div></button>"
  );
}
function notice_show_with_no(notice_group, notice_number) {
  ...
  notice_container.insertAdjacentHTML(
    "beforeend",
    "<big><b onclick='document.getElementById(\"" +
      target_ele_id +
      '").style.display="revert";document.getElementsByClassName("notice")[document.getElementsByClassName("notice").length-1].outerHTML="";\'>Back</b></big>'
  );
  ...
  notice_sec.insertAdjacentHTML(
    "beforeend",
    '<hr style="float: left;"><br><div style="position: relative;"><div class=\'notices_reg_no\'>Ref: ' +
      notice_group.split("_")[0] +
      "/" +
      notices_obj[notice_group][notice_number][0] +
      "</div><div class='notices_date'>Date: " +
      notices_obj[notice_group][notice_number][2].slice(
        notices_obj[notice_group][notice_number][2].length - 2
      ) +
      "/" +
      notices_obj[notice_group][notice_number][2].slice(
        notices_obj[notice_group][notice_number][2].length - 4,
        notices_obj[notice_group][notice_number][2].length - 2
      ) +
      "/" +
      notices_obj[notice_group][notice_number][2].slice(
      ...
      '</div></div><div class="u1 notice_cent_div"><b><big>NOTICE</big></b></div><div class="u1 notice_cent_div">' +
      notice_get_students_group(
        notices_obj[notice_group][notice_number][5]
      ).toUpperCase() +
      '</div><div class="u1 notice_cent_div"><b>' +
      notices_obj[notice_group][notice_number][4] +
      "</b></div>"
  );
  let notice_str = "<article class='cont1 notice_contents'><br>";
  for (
    let i = 8;
    i < 8 + parseInt(notices_obj[notice_group][notice_number][7]);
    i++
  ) {
  ...
  notice_container.insertAdjacentHTML(
    "beforeend",
    '<div style="text-align: center;">.<br>The link for the notice is <a target="_blank" href="/notice/' +
      notice_group.split("_")[0] +
      "/" +
      notices_obj[notice_group][notice_number][0] +
      '">' +
  ...
  document
    .getElementById(target_ele_id)
    .insertAdjacentElement("afterend", notice_container);
...
function notice_get_students_group(student_group) {
  if (student_group == "all all all") {
    ...
    return (
      student_group.replaceAll("all", "").replaceAll("  ", " ").trim() +
      " students"
    );
  }
...
function notice_parse(notice) {
  ...
  notice_sec.insertAdjacentHTML(
    ...
    '<img class="cont1" src="' +
      ...
      notice[0] +
      "</div><div class='notices_date'>Date: " +
      notice[2].slice(notice[2].length - 2) +
      "/" +
      notice[2].slice(notice[2].length - 4, notice[2].length - 2) +
      "/" +
      notice[2].slice(0, notice[2].length - 4) +
      '</div></div><div class="u1 notice_cent_div"><b><big>NOTICE</big></b></div><div class="u1 notice_cent_div">' +
      notice_get_students_group(notice[5]).toUpperCase() +
      '</div><div class="u1 notice_cent_div"><b>' +
      notice[4] +
      "</b></div>"
  );
  ...
  if(notice[6].toString().length>0{
    notice_str += `<img src='https://drive.google.com/thumbnail?id=`+notice[6].toString().split("//")[0]+`' alt='`+"notice_image"+`' style="max-width:500px;max-height=300px;">`
  }
  notice_str += "<br><br></article>";
  notice_sec.insertAdjacentHTML("beforeend", notice_str);
  notice_sec.insertAdjacentHTML(
  ...
  notice_container.insertAdjacentHTML(
    "beforeend",
    "<div style=\"text-align: center;\">.<br><button onclick=\"var wind=window.open();wind.document.write(document.getElementsByClassName('notice_full')[0].outerHTML.replace('https://www.a4u.co.in/images/logo.jpg','https://static-data.a4u.co.in/mech/images/MED-ICEM%20Header.jpg')+'<style>*{font-size: 1.1em !important;float: none !important;}</style>'+document.getElementById('notices_css').outerHTML+document.getElementById('layout_css').outerHTML+document.getElementById('default_css').outerHTML);setTimeout(function() {wind.print();console.log('loaded');},500);\"><span>Print Notice</span></button></div>"
  );
